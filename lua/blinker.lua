...
function blinkON()
  ...
  mytimer = tmr.create()
  mytimer:alarm(1000, tmr.ALARM_AUTO, function()  togLED() end)
end
...
init()
  srv:listen(80,function(conn)
    ...
    fnd = {string.find(payload,"blinker=")}
    ...
    conn:send('<input type="submit" name="blinker" value="ON" > Turn Built in LED on<br><br>\n')
    conn:send('<input type="submit" name="blinker" value="OFF"> Turn Built in LED off<br><br>\n')
    conn:send('<input type="submit" name="blinker" value="Blink"> Blink LED<br><br>\n')
    conn:send('<input type="submit" name="blinker" value="Blinkoff"> Stop LED Blink</form>\n')
    conn:send('</body></html>\n')
